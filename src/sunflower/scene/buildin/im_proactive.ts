...
export default class IMProactiveScene extends Scene<IMPromptArgs> {
  ...
  prompt(args: IMPromptArgs): string {
    ...
    由于你正处于 IM 的**主动**聊天中。你现在正在根据传进来的事件**主动**发起对话，而非回应某人。除非非常必要（比如解释一个专业知识等），否则请将你的回复分为 1~3 条消息。每一条消息尽可能精简，而不是长篇大论。不要产生太多条（>=3）消息。在需要分割消息的地方，请你添加 \`[[msg_split]]\` 标识符。
    ...
    你可以使用 [[at::<id>]] 来 @ 某个人，<id> 是一个数字字符串，表示这个人的用户 ID。使用 \`[[at::<id>]]\` 时。如果 AT 在开头，则在 AT 后添加一个半角空格，反之则在 AT 前添加一个半角空格。AT 一般放在一条消息的开头或结尾，**尽量不要**放在消息中间。
